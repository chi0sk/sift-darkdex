			...
			if button == 1 then
				if combo == 2 then
					if node.Obj:IsA("LuaSourceContainer") then
						--// gotta send a notification bc sift so slow ;-;
						pcall(function()
							StarterGui:SetCore("SendNotification", {
								Title = "! SIFT !",
								Text = "Decompilation of " .. node.Obj,Name .. " Queued, please wait!",
								Button1 = "ok dada",
								Button2 = " :3 "
							})
						end)
...
local function decompileBytecode(bytecode: string, filename: string?): string
	...
	local response = request({
		Url = API .. "/decompile?rootMode=full-script&outputStyle=preserve&outputTier=plain",
		Method = "POST",
		Headers = {
...
--// idk.. sift was returning some weird non UTF-8 bytes??
local function cleanSourceForViewer(source: string): string
	source = tostring(source)
	source = source:gsub("\239\191\189", "_")
	source = source:gsub("[%z\1-\8\11\12\14-\31\127-\255]", function(char)
		return ("\\x%02X"):format(string.byte(char))
	end)
...
local function main()
	...
	ScriptViewer.ViewScript = function(scr)
		...
		codeFrame:SetText(cleanSourceForViewer(source))
		window:Show()
	end
